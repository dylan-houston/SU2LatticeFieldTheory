...
class LatticeMarkovChainTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.metropolis = LatticeMarkovChain(self.lattice, seed=42)

    def test_metropolis_test(self):
        # Delta S < 0 always accepted
        for _ in range(0, 10):
            rand = np.random.rand() * -10
            self.assertTrue(self.metropolis._metropolis_test(rand))

        # Delta S should be accepted if S = 0
        self.assertTrue(self.metropolis._metropolis_test(0))
        ...
        np.random.seed(42)
        self.assertFalse(self.metropolis._metropolis_test(2 * np.log(1 / r)))
        # Delta_S = ln(1/r), should give rejection
        np.random.seed(42)
        self.assertFalse(self.metropolis._metropolis_test(np.log(1 / r)))
        # Try Delta_S = 1/2 * ln(1/r), should give acceptance
        np.random.seed(42)
        self.assertTrue(self.metropolis._metropolis_test(1 / 2 * np.log(1 / r)))
    ...
    def test_matrix_shift_gives_SU2_matrix(self):
        ...
        shifted_matrices = np.vectorize(self.metropolis._metropolis_matrix_shift)(matrices)
    ...
    def test_action_is_real_after_site_metropolis_steps(self):
        ...
        for _ in range(100):
            ...
            self.metropolis._metropolis_site_step(random_x, random_y)
            action = self.lattice.action()
            self.assertEquals(action, np.real(action))

    def test_each_site_pair_visited(self):
        site_pairs = self.metropolis._generate_random_lattice_step_through_sequence()
